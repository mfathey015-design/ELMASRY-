...
GITHUB_TOKEN = "ghp_..."  # ضع التوكن الخاص بك هنا
GITHUB_USER = "mfathey015-design"
REPO_NAME = "ELMASRY-"

BASE_DIR = os.path.abspath(os.path.join(os.getcwd(), "hosted_projects"))
TEMP_DIR = os.path.abspath(os.path.join(os.getcwd(), "temp_uploads"))
...
def run_web():
    try:
        ...
        if "Address already in use" in str(e):
            logger.error(f"⚠️ المنفذ {PORT} مشغول! محاولة استخدام منفذ بديل...")
            try:
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in user_upload_state and "path" in user_upload_state[user_id]:
        ...
        try:
            ...
            # --- [التعديل الجذري والآمن لفك الملفات داخل المجلد بدقة] ---
            with zipfile.ZipFile(state["path"], 'r') as zip_ref:
                for zip_info in zip_ref.infolist():
                    ...
                    # استخراج اسم الملف الأساسي وتجاهل الهياكل الوهمية المتقدمة لضمان استقرار الملفات بالمجلد
                    filename = os.path.basename(zip_info.filename)
                    if not filename:
                        continue
                    
                    target_file_path = os.path.join(extract_path, filename)
            ...
            if not os.path.exists(main_py):
                await msg.edit_text("❌ **خطأ: ملف main.py غير موجود داخل المجلد المستخرج!**", parse_mode='Markdown')
            ...
            if os.path.exists(req_txt):
                ...
                try:
                    ...
                    await msg.edit_text("⚠️ **تحذير: فشل تثبيت بعض المكتبات من requirements.txt**", parse_mode='Markdown')
            ...
            final_text = f"✅ **تم إنشاء مجلد المشروع `{p_name}` وفك الملفات بنجاح!**\n━━━━━━━━━━━━━━━━━━━━━\n🚀 **ملف التشغيل (`main.py`) وملف المكاتب جاهزان الآن داخل المجلد.**\nاختر من قائمة '📁 إدارة الملفات' لتشغيله."
            await msg.edit_text(final_text, parse_mode='Markdown')
        except Exception as e:
            logger.error(f"Upload error: {e}")
            await update.message.reply_text(f"❌ **خطأ أثناء المعالجة:** `{str(e)}`", parse_mode='Markdown')
        return
    ...
    if text == "📦 رفع الملفات":
        await update.message.reply_text(
            ...
            "• `main.py` (ملف التشغيل)\n"
            "• `requirements.txt` (ملف المكاتب)\n"
            "━━━━━━━━━━━━━━━━━━━━━\n"
            "✅ **سيقوم البوت بإنشاء مجلد المشروع وفك الملفات بداخله تلقائياً.**",
            parse_mode='Markdown'
        )
        ...
        if not user_projects:
            await update.message.reply_text("📁 **لا توجد مشاريع مرفوعة بعد.**", parse_mode='Markdown')
            return
        keyboard = []
        ...
        await update.message.reply_text("📁 **قائمة مشاريعك:**", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')

    elif text == "🗑️ حذف المشاريع":
        user_projects = [p for p, d in project_owners.items() if d["u_id"] == user_id]
        if not user_projects:
            await update.message.reply_text("🗑️ **لا توجد مشاريع لحذفها.**", parse_mode='Markdown')
            return
        keyboard = [[InlineKeyboardButton(f"🗑️ {p}", callback_data=f"del_{p}")] for p in user_projects]
        await update.message.reply_text("🗑️ **اختر المشروع المطلوب حذفه:**", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')
        ...
        if text == "🔒 قفل النظام":
            ...
            await update.message.reply_text("⚠️ **يرجى استخدام الأزرار المتاحة.**", parse_mode='Markdown')
    else:
...
async def handle_docs(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not doc.file_name.endswith('.zip'):
        await update.message.reply_text("❌ **يرجى إرسال ملف مضغوط بصيغة .zip فقط!**", parse_mode='Markdown')
        return
    ...
    try:
        ...
        await msg.edit_text("🖋️ **أرسل الآن اسماً لمجلد المشروع (مثال: my_bot):**", parse_mode='Markdown')
    except Exception as e:
        logger.error(f"Download error: {e}")
        await msg.edit_text("❌ **فشل استقبال الملف!**", parse_mode='Markdown')
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "run":
        if p_name in running_processes and running_processes[p_name].poll() is None:
            await query.edit_message_text(f"⚠️ **المشروع `{p_name}` يعمل بالفعل!**", parse_mode='Markdown')
            return
        folder = project_owners.get(p_name, {}).get("path")
        if not folder:
            await query.edit_message_text(f"❌ **المشروع `{p_name}` غير مسجل**", parse_mode='Markdown')
            return
        ...
        if os.path.exists(main_file):
            ...
            try:
                ...
                await msg.edit_text(f"🚀 **تم تشغيل المشروع `{p_name}` بنجاح! 💚**", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')
            except Exception as e:
                await msg.edit_text(f"❌ **خطأ التشغيل:** `{str(e)}`", parse_mode='Markdown')
        else:
            await query.edit_message_text(f"❌ **ملف main.py غير موجود داخل المجلد!**", parse_mode='Markdown')
    elif action == "stop":
        if p_name in running_processes:
            msg = await query.edit_message_text("🛑 جاري إيقاف البوت...")
            ...
            await msg.edit_text(f"🛑 **المشروع `{p_name}` متوقف الآن 💔**", parse_mode='Markdown')
        else:
            await query.edit_message_text(f"⚠️ **المشروع متوقف بالفعل**", parse_mode='Markdown')
    elif action == "viewlogs":
        if not live_logs_enabled:
            await query.answer("❌ السجلات الحية معطلة!", show_alert=True)
            return
        if p_name not in running_processes or running_processes[p_name].poll() is not None:
            await query.answer("❌ المشروع لا يعمل حالياً!", show_alert=True)
            return
        log_msg = await context.bot.send_message(chat_id=update.effective_chat.id, text="📺 **جاري تشغيل السجلات الحية للمجلد...**", parse_mode='Markdown')
        if log_streamer.subscribe(p_name, user_id, update.effective_chat.id, log_msg.message_id):
            ...
            await log_msg.edit_text("❌ **فشل عرض السجلات.**", parse_mode='Markdown')
    elif action == "del":
...
async def health_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    health_data = await get_system_health()
    text = f"🏩 **حالة النظام:** {health_data.get('status', 'ok')}"
    await update.message.reply_text(text, parse_mode='Markdown')

async def server_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id): return
    await update.message.reply_text(f"🌎 **المنفذ النشط:** {PORT}", parse_mode='Markdown')
...
async def projects_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(user_id):
        user_projects = [p for p, d in project_owners.items() if d["u_id"] == user_id]
        if not user_projects:
            await update.message.reply_text("📮 **ليس لديك أي مشاريع.**", parse_mode='Markdown')
            return
        keyboard = [[InlineKeyboardButton(p, callback_data=f"manage_{p}")] for p in user_projects]
        await update.message.reply_text("📦 **مشاريعك:**", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')

async def lock_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id): return
    global bot_locked
    bot_locked = True
    await update.message.reply_text("🔒 **تم قفل البوت العام.**", parse_mode='Markdown')

async def unlock_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id): return
    global bot_locked
    bot_locked = False
    await update.message.reply_text("🔓 **تم فتح البوت العام.**", parse_mode='Markdown')
...
def main():
    ...
    async def post_init(app):
        await app.bot.set_my_commands([BotCommand("start", "🚀 تشغيل لوحة التحكم")])
